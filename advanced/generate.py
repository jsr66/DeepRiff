...
def create_generation_batch(model, num_words, random_choice_frequency, 
    ...
    t=TEXT.numericalize(s, device=-1) 
...
def compose(model_to_load, training, test, train, gen_size, sample_freq, chordwise, 
    ...
    for i in range(len(results)):
        write_mid_final(results[i], str(i).zfill(2)+".mid", sample_freq, note_offset, out, chordwise)
        write_pdf_final(results[i], str(i).zfill(2), sample_freq, note_offset, out_pdf, chordwise)
    ...
    for i in range(len(musical_prompts)):
        write_mid_final(musical_prompts[i], "prompt"+str(i).zfill(2)+".mid", sample_freq, note_offset, out, chordwise)
...
if __name__ == "__main__":
    ...
    compose(args.model, args.training, test, train, args.size, sample_freq, args.chordwise,
