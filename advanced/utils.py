...
def create_paths():
    PATHS={}
    PATHS['data']=Path('/home/ubuntu/application/advanced/data/')      
    PATHS['models']=Path('/home/ubuntu/application/advanced/models/')
...
def load_pretrained_model(model_to_load, PATHS, training, bs):
    ...
    lm.model.load_state_dict(torch.load(PATHS["generator"]/mod_name, map_location=lambda storage, loc: storage))
...
def write_pdf(s, filename, output_folder):
    try:
    	fp = s.write('lily.pdf', fp=output_folder/filename)
    except:
...
def write_mid_final(stream, fname, sample_freq, note_offset, out_midi, chordwise):
    stream_out=string_inds_to_stream(stream, sample_freq, note_offset, chordwise)
    write_midi(stream_out, fname, out_midi)

def write_pdf_final(stream, fname, sample_freq, note_offset, out_pdf, chordwise):
    stream_out=string_inds_to_stream(stream, sample_freq, note_offset, chordwise)
    write_pdf(stream_out, fname, out_pdf)
